...
add_to_cart <- function(book) {
  ...
  message(paste(book_table$Title, "has been added to your cart."))
}
...
if(!is.null(results)) {
  while(TRUE) {
    ...
    if(add_to_cart_input == "y") {
      ...
      query <- paste0("INSERT INTO Cart_Table (Title , isbn, quantity) VALUES ('", book_table$title, "', '",book_table$isbn, "', 1)")
